...
site = Blueprint(
    "site", __name__, template_folder="site_templates"
)  # telling your blueprint where to load the html files
...
def shop():
    ...
    shop_stats = {
        "cars": len(shop),
        "sales": sum(
            [order.order_total for order in orders]
        ),  # order totals was total cost of that specific order
        "customers": len(customers),
    }

    return render_template(
        "shop.html", shop=shop, stats=shop_stats
    )  # basically displaying our shop.html page
