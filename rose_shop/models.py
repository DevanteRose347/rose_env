...
)  # allows us to generate a hashed password
from flask_sqlalchemy import (
    SQLAlchemy,
)  # allows our database to read our classes/objects as tables/rows
from flask_login import (
    UserMixin,
    LoginManager,
)  # allows us to load a current logged in user
...
def load_user(user_id):
    return User.query.get(
        user_id
    )  # this queries our database & brings back the user with the same id
...
class User(db.Model, UserMixin):
    ...
    def __init__(self, username, email, password, first_name="", last_name=""):
        ...
        self.password = self.set_password(
            password
        )  # method to hash our password for security
...
class Car(db.Model):
    ...
    def __init__(
        self, make, year, model, color, price, quantity, image="", description=""
    ):
    ...
    def decrement_quantity(self, quantity):
        ...
        return (
            self.quantity
        )  # all methods need to return otherwise the object attribute doesnt get updated
...
class Customer(db.Model):
    ...
    carord = db.relationship(
        "CarOrder", backref="customer", lazy=True
    )  # backref is just how are these related, lazy means a Customer can exist without the CarOrder table
...
class CarOrder(db.Model):
    ...
    def update_quantity(
        self, quantity
    ):  # method used for when customers update their order quantity of a specific product
        self.quantity = int(quantity)
        return self.quantity
...
cars_schema = CarSchema(
    many=True
)  # this for passing multiple cars, list of dictionaries
